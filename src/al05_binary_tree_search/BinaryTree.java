...
public class BinaryTree <K,V>{
	...
	private Node<K, V> root; // 루트노드(제일 꼭대기에 있는 노드), 여기서 root는 전역변수
	BinaryTree(){
		root = null; // 루트노드 초기화
	}
	
	public int comp(K key1, K key2) {
		// key1-key2   0: key1과 key2가 같다, 양수: key1이 크다, 음수: key2가 크다
		return ((Comparable<K>)key1).compareTo(key2);
	}
	
	// 노드추가(위치를 찾아서 노드추가)
	public void addNode(Node<K,V> node, K key, V data) { //여기서 root는 지역변수
		// 비교
		int result = comp(key, root.getKey());
		if(result==0){ // 이미 있는 키값일때
			return;
		} else if(result<0) { // 음수: 왼쪽노드를 검색
			if(node.left==null) { // node의 왼쪽이 비어있으면, 새로운 node를 node왼쪽에 추가 
	...
	public boolean remove(K key) {
		...
		boolean isLeftChild = true; // 왼쪽or오른쪽 노드인지 정보가 필요함
		...
		}else if(point.left==null) {// left노드가 없을때(삭제할 노드의 right을 부모노드의 left에 대입한다)
			// 삭제할 노드가 root면 오른쪽 노드를 root
			if(point==root) {
				root = point.right;
			}else if(isLeftChild) {
		...
		}else if(point.right==null) {// 삭제할 노드가 right에 없을때
			if(point==root) {
				root = point.left;
			}else if(isLeftChild) {
	...
	public V search(K key) {
		Node<K,V> point = root; // root, left, right
		
		while(true) {
			if(point==null) {
				return null; // 노드가 없을때
			}
			// 입력받은 키와 point에 있는 키 크기를 구하여
